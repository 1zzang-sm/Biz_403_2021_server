...
// /book : 요청 path(경로)
// ?변수=값 : Query 요청값
@WebServlet("/book/*") // ㅇㅇ/* > ㅇㅇ뒤에 무엇이던 첨가가능  
public class BookController extends HttpServlet {
	...
	/**
	 * WAS 관련 프로젝트를 수행할때 클래스를 만들고 서버를 한번 Run 한 후에 다시 클래스 코드를 변경하면 Tomcat이 자동으로 재
	 * 시작된다. 그때 Tomcat이 내부적으로 클래스의 정보를 참조하는 일종의 Key 값
	 */
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 *   이때 추가된 sub Path를 추출할때는 req.getPathInfo()를 사용하여 추출한다.
		 */
	...
	//	Integer.valueOf("A0001"); 일부러 오류내는 코드
		
		PrintWriter out = null; //resp.getWriter();
		out.println("Welcom to my home : ");
	...
	private void callService() {
		...
		// 도서 1개 정보 조회
		BookVO retVO = bService.findById("9700000");
		
		// 도서정보 변경
		bService.update(bookVO);
		
		// 도서정보 데이터 1개 삭제 
		bService.delete("9700000");
