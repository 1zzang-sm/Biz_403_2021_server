 ...
 * 공통으로 처리해야 하는 것들을 미리 Filter에서 처리를 하고 처리된 결과를 Controller에게 전달하는 Tomcat WAS의 처리 절차
 * 
 * Filter에서 처리된 설정은 모든 Controller에 일일이 설정하는 것과 같은 효과를 발휘한다.
 * 
 * login과 같은 처리는 각가의 Controller가 사용자의 요청을 받으면
 * 항상 login이 되어 있는지 검사하고 그에 따른 처리를 수행해야 하는데
 * 
 * 그러한 절차를 미리 Filter에서 처리하고 login 여부에 따라 
 * Controller로 전달할지 안할지를 판단하여 미리 한번에 처리 할 수 있다.
 */
