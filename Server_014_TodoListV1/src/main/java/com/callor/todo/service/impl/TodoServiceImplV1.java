...
public class TodoServiceImplV1 implements TodoService {
	 ...
	 * 메타데이터
	 * 
	 * 객체(Entity, Replation) DataBase를 만들고 Table을 생성한 후 Insert 수행하면 실제상황의 데이터가
	 * Table에 저장된다. INSERT를 수행한 후 SELECT를 수행하여 조회할 수 있는 데이터들
	 * 
	 * 메타데이터(meta data) 생성한 Database, Table, Sequence, view ... 이 어떤 구조로 만들어져 있는가 등의
	 * 정보를 메타데이터라고 한다. ex) myDB라는 DataBase에 몇개의 table이 있는가? 등의 정보를 알고 싶으면 myDB
	 * 메타데이터를 참조하면 된다.
	 * 
	 */
	...
	public List<Map<String, Object>> selectAll() {
		// TODO 전체 조회할 SelectAll List로 담는다.
	...
	public Integer insert(Map<String, Object> tdVO) {
		...
		// 1.8 ~ 10v까지 갑자기 등장 버전에 따라 사용하지 않아도 되지만 호환성을 위해 사용
		// 현재 날짜 가져오기
		Date date = new Date(System.currentTimeMillis());
		// 날짜를 문자열화 하기 위한 보조 클래스
		SimpleDateFormat sd = new SimpleDateFormat("yyyy-MM-dd");
		// 시간을 문자열화 하기 위한 보조 클래스
