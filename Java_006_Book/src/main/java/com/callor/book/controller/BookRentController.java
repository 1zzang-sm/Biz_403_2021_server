	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (subPath.equals("/seq")) {
			...
			if (bu_name == null || bu_name.equals("")) {
				...
				// page1.jps 파일을 열고 BUYERS 변수와 함께 Rendering을 하여 HTML 코드를 생성하라
			...
			// BUYER에 담긴 회원정보를 page2.jpg에 보여라
			...
			String bk_code = req.getParameter("bu_code");
			if (bk_title == null || bk_title.equals("")) {
				out.println("도서명입력");
				out.close();
			} else {
				// 회원정보를 한번더 조회
				BuyerDTO buDTO = buService.findById(bk_code);
			...
			// INSERT를 수행하기 위해 VO를 만들고 web에서 전달받은 도서ISBN과 회원CODE를 Setting
