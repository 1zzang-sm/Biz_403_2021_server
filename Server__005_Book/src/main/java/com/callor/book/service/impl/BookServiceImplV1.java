...
public class BookServiceImplV1 implements BookService {
	...
	public void insert(BookVO bookVO) {
		...
		sql += "(bk_isbn, bk_title, bk_code, bk_acode, bk_date, bk_price, bk_pages)";
		sql += "VALUES(?,?,?,?,?,?,?)";

		// sql = "(" + bookVO.getBk_isbn() + "," + bookVO.getBk_title() .....ice() + ")"

		// String type의 SQL 명령문을 Oracle에 전송하기 위해 Packet 로 만들기
		PreparedStatement pStr = null;
		try {
			pStr = dbConn.prepareStatement(sql);
			pStr.setString(1, bookVO.getBk_isbn());
			pStr.setString(2, bookVO.getBk_title());
			pStr.setString(3, bookVO.getBk_code());
